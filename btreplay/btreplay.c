...
static int naios = 12;                  // Number of AIOs per thread
...
static size_t pattern_size = 0;

static int threads_number = -1;
...
*/
        if (!pattern) {
                for (size_t i = 0; i < bsize; i += pgsize) {
                        buf[i] = 0;
                }
        } else {
                for (size_t i = 0; i < bsize; i += pattern_size) {
                        memcpy(buf + i, pattern, minl(pattern_size, bsize - i));
                }
        }
}
 ...
 */
static inline void setup_signal(int signum, __sighandler_t handler)
{
...
static int reap_wait_aios(struct thr_info *tip)
{
        ...
        if (!is_reap_done(tip)) {
                ...
                while (tip->naios_out == 0 && !is_reap_done(tip)) {
                        tip->reap_wait = 1;
        ...
        return is_reap_done(tip) ? 0 : tip->naios_out;
}
...
static void *replay_sub(void *arg)
{
        ...
        oflags = 0;
#endif
        tip->ofd = open(path, O_RDWR | __O_DIRECT | oflags);
        if (tip->ofd < 0) {
...
#define S_OPTS  "c:d:Fhi:I:M:Np:n:x:t:vVW"
...
int main(int argc, char *argv[])
{
        ...
        list_for_each_safe(pp, qq, &input_files) {
                struct thr_info* rr = list_entry(pp, struct thr_info, head);
                pthread_cond_signal(&rr->cond);
        }
